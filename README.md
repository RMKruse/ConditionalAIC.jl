...
[![CI](https://github.com/RMKruse/ConditionalAIC.jl/actions/workflows/CI.yml/badge.svg)](https://github.com/RMKruse/ConditionalAIC.jl/actions/workflows/CI.yml)
...
compares the full cAIC within a fit-discrepancy-justified tolerance. The
mathematical specification for each estimator lives in [`docs/math/`](docs/math/).
...
If you use `ConditionalAIC.jl` in published work, please cite the software (see
[`CITATION.cff`](CITATION.cff)) together with the two methodology papers it
implements:

```bibtex
@article{greven2010caic,
...
@article{saefken2021caic4,
  author  = {S\"afken, Benjamin and R\"ugamer, David and Kneib, Thomas and Greven, Sonja},
...
@article{kruse2022modelavg,
  author  = {Kruse, Ren\'e-Marcel and Silbersdorff, Alexander and S\"afken, Benjamin},
  title   = {Model averaging for linear mixed models via augmented {L}agrangian},
...
Development decisions and the R-divergence log live alongside the source:

- [`PARITY.md`](PARITY.md) — the authoritative parity matrix against `cAIC4`.
- [`DECISIONS.md`](DECISIONS.md) — every place `ConditionalAIC.jl` legitimately
  diverges from `cAIC4`, each with its justified tolerance or behaviour.
- [`docs/adr/`](docs/adr/) — architecture decision records.
- [`docs/math/`](docs/math/) — the mathematical specification for each estimator.
