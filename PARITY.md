...
| — (same, GLMM dispatch) | `caic(m::GeneralizedLinearMixedModel; …)` | M3 | 🟢 | Poisson (Chen-Stein, `method=:auto`) and Bernoulli (Efron Steinian, `method=:auto`) implemented and assembly-tested (#31). `method=:bootstrap` conditional-draw override works for all M3 families. Binomial and other non-Poisson/Bernoulli families require `method=:bootstrap`. Full-singularity fallback (ρ=rank(X)) unchanged from #27. Level-2 tolerance ungrilled for GLMM (fit-discrepancy band vs. cAIC4 — df correctness validated in #28/#29/#30 isolation). |
...
| Bernoulli / binomial GLMM | Efron's estimator (`biasCorrectionBernoulli`) | M3 | 🟢 | Influence-based, *not* Greven–Kneib. `dof_glmm_bernoulli` (#29): Level-1 kernel validated; Level-2 fixture (atol=1e-3). Wired into `caic` assembly (#31). Multi-trial Binomial routes to bootstrap. |
| other GLMM families | conditional bootstrap (`conditionalBootstrap`) | M3 | 🟢 | `dof_glmm_bootstrap` (#30): conditional draws `yᵢ ~ f(μ̂ᵢ)` via `glmmconddraw` (Poisson/Bernoulli/Binomial; ADR-0005), n×B refits, Efron penalty with σ=1. Wired into `caic(m; method=:bootstrap)` (#31). Level-2 fixture vs. cAIC4 `conditionalBootstrap` at atol=2.0. |
...
| `conditionalBootstrap` | `method=:bootstrap` | 🟢 | Gaussian: parametric bootstrap (#12). GLMM: conditional bootstrap (#30, #31); validated vs. cAIC4 `conditionalBootstrap` (atol=2.0). `rng` kwarg for reproducibility. |
