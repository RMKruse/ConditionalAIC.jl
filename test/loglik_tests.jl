@testitem "condloglik matches the hand-computed Gaussian conditional log-likelihood" tags = [
    ...
    @test ConditionalAIC.Loglik.condloglik([3.0], [3.0], 1.0) ≈ -0.5 * log(2π) rtol = 1e-6 atol =
        1e-10
    ...
    @test ConditionalAIC.Loglik.condloglik(y, ŷ, σ) ≈ ref rtol = 1e-6 atol = 1e-10
end
...
@testitem "condloglik is type-stable and generic over T" tags = [:loglik] begin
    ...
    @test (@inferred ConditionalAIC.Loglik.condloglik(y, ŷ, σ)) ≈ ref

    # Float32 in ⟹ Float32 out (no silent promotion to Float64).
    y32, ŷ32, σ32 = Float32.(y), Float32.(ŷ), 0.8f0
    @test ConditionalAIC.Loglik.condloglik(y32, ŷ32, σ32) isa Float32
    @test (@inferred Float32 ConditionalAIC.Loglik.condloglik(y32, ŷ32, σ32)) ≈ ref rtol =
        1e-5
end
...
@testitem "condloglik rejects σ̂ outside its domain and mismatched lengths" tags = [:loglik] begin
    ...
    @test_throws DomainError ConditionalAIC.Loglik.condloglik(y, ŷ, 0.0)
    @test_throws DomainError ConditionalAIC.Loglik.condloglik(y, ŷ, -1.0)
    @test_throws DomainError ConditionalAIC.Loglik.condloglik(y, ŷ, NaN)

    # y and ŷ must index alike.
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik(y, [1.0, 2.0], 1.0)
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik([1.0, 2.0], ŷ, 1.0)
end
...
@testitem "condloglik handles empty input, single obs, perfect fit, and non-finite data" tags = [
    ...
    @test ConditionalAIC.Loglik.condloglik(Float64[], Float64[], 1.0) == 0.0

    # A single observation is the univariate normal log-density.
    @test ConditionalAIC.Loglik.condloglik([2.0], [0.5], 1.5) ≈
        -0.5 * log(2π) - log(1.5) - (2.0 - 0.5)^2 / (2 * 1.5^2) rtol = 1e-6 atol = 1e-10

    # A perfect fit (e = 0) is the finite maximum −(n/2)·log(2π) − n·log σ̂.
    n, σ = 5, 2.0
    @test ConditionalAIC.Loglik.condloglik(fill(3.0, n), fill(3.0, n), σ) ≈
        -(n / 2) * log(2π) - n * log(σ) rtol = 1e-6 atol = 1e-10

    # Non-finite data propagates rather than being silently dropped.
    @test isnan(ConditionalAIC.Loglik.condloglik([NaN, 1.0], [0.0, 1.0], 1.0))
    @test isinf(ConditionalAIC.Loglik.condloglik([Inf, 1.0], [0.0, 1.0], 1.0))
end
...
@testitem "condloglik_poisson matches hand-computed Poisson conditional log-likelihood" tags = [
    ...
    @test ConditionalAIC.Loglik.condloglik_poisson([1.0], [1.0]) ≈ -1.0 rtol = 1e-6 atol =
        1e-10
...
@testitem "condloglik_poisson is type-stable and generic over T" tags = [:loglik] begin
    ...
    @test (@inferred ConditionalAIC.Loglik.condloglik_poisson(y, μ)) ≈ ref

    y32, μ32 = Float32.(y), Float32.(μ)
    @test ConditionalAIC.Loglik.condloglik_poisson(y32, μ32) isa Float32
    @test (@inferred Float32 ConditionalAIC.Loglik.condloglik_poisson(y32, μ32)) ≈
        Float32(ref) rtol = 1e-4
end
...
@testitem "condloglik_poisson rejects invalid μ̂ and mismatched lengths" tags = [:loglik] begin
    ...
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_poisson(y, [0.0, 1.0])
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_poisson(y, [-0.5, 1.0])
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik_poisson(y, [1.0])
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik_poisson([1.0], y)
end
...
@testitem "condloglik_poisson handles empty input, y=0 entries, and non-finite data" tags = [
    ...
    @test ConditionalAIC.Loglik.condloglik_poisson(Float64[], Float64[]) == 0.0

    # y_i = 0 with μ̂_i = c: xlogy(0, c) = 0, so ℓ = −c − log(0!) = −c.
    @test ConditionalAIC.Loglik.condloglik_poisson([0.0], [2.5]) ≈ -2.5 rtol = 1e-6 atol =
        1e-10

    # Non-finite counts propagate.
    @test isnan(ConditionalAIC.Loglik.condloglik_poisson([NaN, 1.0], [1.0, 1.0]))
end
...
@testitem "condloglik_bernoulli matches hand-computed Bernoulli conditional log-likelihood" tags = [
    ...
    @test ConditionalAIC.Loglik.condloglik_bernoulli([1.0], [0.5]) ≈ -log(2.0) rtol = 1e-6 atol =
        1e-10
...
@testitem "condloglik_bernoulli is type-stable and generic over T" tags = [:loglik] begin
    ...
    @test (@inferred ConditionalAIC.Loglik.condloglik_bernoulli(y, μ)) ≈ ref

    y32, μ32 = Float32.(y), Float32.(μ)
    @test ConditionalAIC.Loglik.condloglik_bernoulli(y32, μ32) isa Float32
    @test (@inferred Float32 ConditionalAIC.Loglik.condloglik_bernoulli(y32, μ32)) ≈
        Float32(ref) rtol = 1e-4
end
...
@testitem "condloglik_bernoulli rejects invalid μ̂ and mismatched lengths" tags = [:loglik] begin
    ...
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_bernoulli(y, [0.0, 0.5])
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_bernoulli(y, [0.5, 1.0])
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_bernoulli(y, [-0.1, 0.5])
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_bernoulli(y, [0.5, 1.1])
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik_bernoulli(y, [0.5])
...
@testitem "condloglik_bernoulli handles empty input and non-finite data" tags = [:loglik] begin
    # Empty input is the empty sum.
    @test ConditionalAIC.Loglik.condloglik_bernoulli(Float64[], Float64[]) == 0.0

    # Non-finite data propagates.
    @test isnan(ConditionalAIC.Loglik.condloglik_bernoulli([NaN, 1.0], [0.5, 0.5]))
end
...
@testitem "condloglik_binomial matches base-R dbinom (multi-trial)" tags = [:loglik] begin
    ...
    @test ConditionalAIC.Loglik.condloglik_binomial([0.5], [0.5], [2.0]) ≈ -log(2.0) rtol =
        1e-6 atol = 1e-10

    # base R: dbinom(2, 3, 0.5, log=TRUE) = −0.9808293 (k=2, n=3, μ̂=0.5; y = 2/3).
    @test ConditionalAIC.Loglik.condloglik_binomial([2 / 3], [0.5], [3.0]) ≈
        -0.9808292530117262 rtol = 1e-6 atol = 1e-10
    ...
    μ = [0.5, 0.3, 0.6]
    y = k ./ n
    @test ConditionalAIC.Loglik.condloglik_binomial(y, μ, n) ≈ -3.628836 rtol = 1e-6 atol =
        1e-5
    ...
    @test ConditionalAIC.Loglik.condloglik_binomial(y, μ, n) ≈ ref rtol = 1e-6 atol = 1e-10
end
...
@testitem "condloglik_binomial reduces to Bernoulli when all nᵢ = 1" tags = [:loglik] begin
    ...
    μ = [0.3, 0.7, 0.9]
    n = ones(3)
    @test ConditionalAIC.Loglik.condloglik_binomial(y, μ, n) ≈
        ConditionalAIC.Loglik.condloglik_bernoulli(y, μ) rtol = 1e-12
end
...
@testitem "condloglik_binomial is type-stable and generic over T" tags = [:loglik] begin
    ...
    @test (@inferred ConditionalAIC.Loglik.condloglik_binomial(y, μ, n)) ≈ ref rtol = 1e-5

    y32, μ32, n32 = Float32.(y), Float32.(μ), Float32.(n)
    @test ConditionalAIC.Loglik.condloglik_binomial(y32, μ32, n32) isa Float32
    @test (@inferred Float32 ConditionalAIC.Loglik.condloglik_binomial(y32, μ32, n32)) ≈
        Float32(ref) rtol = 1e-3
end
...
@testitem "condloglik_binomial rejects invalid inputs" tags = [:loglik] begin
    ...
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_binomial(y, [0.0, 0.5], n)
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_binomial(y, [0.5, 1.0], n)
    # Trial counts must be strictly positive.
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_binomial(
        y, [0.5, 0.5], [0.0, 2.0]
    )
    @test_throws DomainError ConditionalAIC.Loglik.condloglik_binomial(
        y, [0.5, 0.5], [-1.0, 2.0]
    ...
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik_binomial(y, [0.5], n)
    @test_throws DimensionMismatch ConditionalAIC.Loglik.condloglik_binomial(
        y, [0.5, 0.5], [2.0]
    )
end
...
@testitem "condloglik_binomial handles empty input and boundary counts" tags = [:loglik] begin
    # Empty input is the empty sum.
    @test ConditionalAIC.Loglik.condloglik_binomial(Float64[], Float64[], Float64[]) == 0.0
    ...
    @test ConditionalAIC.Loglik.condloglik_binomial([0.0], [0.25], [4.0]) ≈ 4 * log(0.75) rtol =
        1e-10
    @test ConditionalAIC.Loglik.condloglik_binomial([1.0], [0.5], [3.0]) ≈ 3 * log(0.5) rtol =
        1e-10
end
